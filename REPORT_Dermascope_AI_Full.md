...
```
FiLM(F_i) = γ_i ⊙ F_i + β_i
```

Where:
- `F_i` is the i-th feature map from the visual encoder (shape: `[B, C, H, W]`).
- `γ_i` (gamma) and `β_i` (beta) are learned affine parameters, **predicted from the conditioning input** (our metadata).
...
def train_expert_phase1(model, train_dl, val_dl, save_path, epochs=15, patience=6):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            patience_counter += 1
            print(f"🏁 Ep {epoch+1} | Val Loss: {avg_val_loss:.4f} | Acc: {accuracy:.2f}% (⚠️ {patience_counter}/{patience})")
...
Ep 03 | Val Loss: 0.0433 | Acc: 74.85%  (⚠️ 1/6)
...
## Chapter 11: Explainability — Filtered Grad-CAM
...
2. **Low Spatial Resolution of Grad-CAM:** EfficientNet-B4's spatial resolution at the final convolutional layer is 16×16, requiring 32× upsampling. This inherently limits the spatial precision of explanations.
